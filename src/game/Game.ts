...
export class Game {
  ...
  canPlayCard(card: Card): boolean {
    ...
    if (
      topCard.color === CardColor.Black &&
      this.sumToDraw === 0 &&
      card.color === this.currentWildColor
    )
      return true;
    ...
    if (
      topCard.color === CardColor.Black &&
      topCard.action === CardAction.Draw4 &&
      card.action === CardAction.Draw2 &&
      card.color === this.currentWildColor
    )
      return true;
    ...
    if (
      this.sumToDraw > 0 &&
      card.action === CardAction.Draw2 &&
      topCard.action === CardAction.Draw2
    )
      return true;
  ...
  playCard(playerId: number, card: Card) {
    if (!this.hasSkipped) {
      ...
      this.currentPlayer =
        (this.currentPlayer + this.direction) % this.players.length;
      return;
    }
    if (this.sumToDraw > 0) {
    ...
    db.query(
      ...
      .catch((error: any) => {
        console.error(
          "Error al actualizar la carta jugada en la base de datos:",
          error,
        );
      });
    ...
    db.query(
      ...
      .catch((error: any) => {
        console.error(
          "Error al actualizar el estado de la partida en la base de datos:",
          error,
        );
      });
  ...
  cantPlay(playerId: number) {
    const player = this.players[playerId];
    let card = this.drawDeck.pop() as Card;
        ...
        for (const player of this.players) {
          db.query(
            `
					...
					VALUES ($1, $2, $3)`,
            [this.gameId, player.id, JSON.stringify(player.hand)],
          );
        }
      })
      .catch((error: any) => {
